<?php
// qTools Widget:
// Latest from category/categories


// Widget Registration
add_action( 'widgets_init', create_function( '', 'return register_widget("qToolsLatestCategory");' ) );


// Widget Class
if ( ! class_exists( 'qToolsLatestCategory' ) ) {
	class qToolsLatestCategory extends WP_Widget {
		public function __construct() {
			parent::__construct(
				'qtools_latest_category', // Base ID
				__( 'qTools: Latest from Category', 'qtools' ), // Name
				array( 'description' => __( 'Latest posts from category', 'qtools' ), ) // Args
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$latest_args = array(
				...
				'post_status' => 'publish',
				'category__in' => array_map ( 'absint', $instance['categories'] )
			);
		...
		public function form( $instance ) {
			$instance   = wp_parse_args( (array) $instance, array( 'title' => __( 'Latest in category', 'qtools' ) ) );
			$title      = sanitize_text_field( $instance['title'] );
			$categories = ! empty( $instance['categories'] ) ? $instance['categories'] : null;
			...
			<p>
				<label style="display: block;" for="<?php echo $this->get_field_id( 'categories' ); ?>"><strong><?php _e( 'Choose category(-ies):', 'qtools' ); ?></strong></label>
				<select multiple class="widefat" id="<?php echo $this->get_field_id( 'categories' ); ?>" name="<?php echo $this->get_field_name( 'categories' ); ?>[]">
				<?php
				$parent_args = array(
					'orderby'		=> 'name',
					'order'			=> 'ASC',
				...
				$parent_cats = get_terms( 'category', $parent_args );

				foreach ($parent_cats as $parent_cat) {
					$option_selected = in_array( $parent_cat->term_id, $categories ) ? ' selected="selected"' : '';
					$option_output = '<option value="' . $parent_cat->term_id . '"' . $option_selected . '>' . $parent_cat->name . '(' . $parent_cat->count . ')' . '</option>';
					echo $option_output;
					...
					foreach ($child_cats as $child_cat) {
						$option_selected = in_array( $child_cat->term_id, $categories ) ? ' selected="selected"' : '';
						$option_output = '<option value="' . $child_cat->term_id . '"' . $option_selected . '>&mdash; ' . $child_cat->name . '(' . $child_cat->count . ')' . '</option>';
						echo $option_output;
					}
				}
				?>
