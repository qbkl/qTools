...
// Widget Registration
add_action( 'widgets_init', create_function( '', 'return register_widget("qToolsSameAuthor");' ) );


// Widget Class
if ( ! class_exists( 'qToolsSameAuthor' ) ) {
	class qToolsSameAuthor extends WP_Widget {
		public function __construct() {
			parent::__construct(
				'qtools_same_author', // Base ID
				__( 'qTools: From Same Author', 'qtools' ), // Name
				array( 'description' => __( 'Latest posts from same author', 'qtools' ), ) // Args
			);
		}
		...
		public function form( $instance ) {
			$instance = wp_parse_args( (array) $instance, array( 'title' => __( 'By same author', 'qtools' ) ) );
			...
			<p>
				<strong style="display: block;"><?php _e( 'Randomize:', 'qtools' ); ?></strong>
				<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'random' ); ?>" name="<?php echo $this->get_field_name( 'random' ); ?>"<?php checked( $random ); ?> /> <label for="<?php echo $this->get_field_id( 'random' ); ?>"><?php _e( 'Display random posts?', 'qtools' ); ?></label>
			</p>
			<?php 
		}
